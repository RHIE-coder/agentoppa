...
**판정:** `acceptance`·`contract` = 기계(contract: `.harness/<하네스>/artifacts/<feature>/` 산출물의 헤더(§2)·연결(§4 consumes→앞 produces) 점검. 신선도 §3 lock 대조는 미구현 — agent-engineer validator 몫으로 남김).

## 절차
1. `setup` → 2. generate(하네스) → 3. run: 기능 1개 구현(예: 게시판 댓글 `POST /posts/:id/comments` + 테스트). 단계 산출은 `.harness/<하네스>/artifacts/<feature>/` 에 쌓인다. → 4. `judge`.
