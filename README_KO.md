...
├── .harness/<하네스>/  ← Project 층 = 이 프로젝트의 구현·바인딩
...
| **Project** | *이* 프로젝트의 구현. Core의 빈자리를 이 프로젝트 것으로 채움 | `.harness/<하네스>/` |

> **재사용의 비결 = "값을 안 박는 것".** Core는 `테스트 도구` 같은 프로젝트 값을 굳히지 않고 **빈자리**로 비워 둡니다. 그 값은 실행 시점에 `.harness/<하네스>/`에서 읽습니다. 그래서 한 Core를 web·mobile·go 어디서나 그대로 씁니다.
...
```bash
node ./plugins/agentoppa/bin/build-skills.mjs <core-authoring-project> <하네스이름>
# → .harness/<하네스이름>/ 를 읽어 .agentoppa/ 에 재사용 Core 한 벌로 컴파일합니다.
```

> 하네스 이름 = `.harness/` 아래 폴더 이름(= Core 이름)입니다. 안 적으면 빌드가 `.harness-main`(또는 환경변수 `HARNESS_MAIN`)에서 활성 하네스를 읽습니다. 여러 하네스가 `.harness/` 아래 공존해도, 도는 건 항상 하나입니다.
...
새 프로젝트는 단계를 **복사하지 않습니다.** Core가 든 `setup` 스킬이 `.harness/<하네스>/config.yaml`을 *스스로 깔아 줍니다* — **AgentOppa 없이.**
