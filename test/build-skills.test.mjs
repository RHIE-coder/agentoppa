...
function compileInTemp(prefix) {
  ...
  // input 전체(.harness/<demo>/ + .harness-main 선택기)를 복사 → build 가 .harness-main 로 활성 하네스를 집는다.
  cpSync(join(fx, "input"), work, { recursive: true });
...
test("build-skills golden — 컴파일 SKILL.md 가 스냅샷과 일치 (값·능력 모두 런타임읽기)", () => {
  ...
  try {
    ...
    assert.ok(testSkill.includes("`.harness/demo/config.yaml`"), "런타임읽기 안내(.harness/<core>/config.yaml 읽어 실행)가 없음");
    ...
    for (const [nm, md] of [["spec", specSkill], ["test", testSkill]]) {
      ...
      assert.ok(md.includes(".harness/demo/artifacts/<작업폴더>/"), `${nm}: 작업폴더가 <작업폴더> 자리표시로 안 남음(네임스페이스 .harness/demo/)`);
    }
...
test("build-skills — 빌드된 Core 가 소비 프로젝트의 .harness 를 자급한다 (setup·interface·scaffold, AgentOppa 없이)", () => {
  // 새 모델: 빌드된 Core(플러그인)가 *스스로* 소비 프로젝트의 .harness/<core>/config.yaml 을 깐다 → 프로젝트B는 AgentOppa 불필요.
  ...
  try {
    ...
    // 2) 소비 프로젝트(빈 디렉터리, AgentOppa 없음)가 Core 의 scaffold.mjs 만으로 .harness/<core>/config.yaml 을 깐다.
    ...
    const cfgPath = join(consumer, ".harness/demo/config.yaml");
    assert.ok(existsSync(cfgPath), "소비 프로젝트 .harness/demo/config.yaml 미생성");
...
function buildInline(prefix, name, files) {
  const work = mkdtempSync(join(tmpdir(), prefix));
  ...
  // 하네스 이름을 2번째 인자로 넘긴다(= selector 대체) → .harness/<name>/config.yaml 을 읽어 빌드.
  const r = spawnSync(process.execPath, [buildSkills, work, name], { encoding: "utf8" });
  return { work, r };
}

test("build-skills 골격 결함 가드 — 프로젝트 훅 이관(안 덮음)·always-on 주어·self-gate 중복·README 적재경로", () => {
  const { work, r } = buildInline("bs-guard-", "guardcore", {
    ".harness/guardcore/config.yaml": "harness: guardcore\nsync: strict\nphases:\n  - solo\n",
    ".harness/guardcore/project/phases/solo.md":
      "---\nname: solo\ndesc: 단독 단계.\nconsumes: ~\nproduces: out\nwhen: 직행 경로일 때만\ngate: \"무언가 green\"\n---\n본문 한 줄. → {next}\n",
    // 프로젝트 저작 훅 — 커스텀 세션훅 + 스크립트. build 가 기본 게이트로 덮으면 안 된다.
    ".harness/guardcore/project/hooks/hooks.json":
      '{\n  "hooks": {\n    "SessionStart": [\n      { "hooks": [{ "type": "command", "command": "node \\"${CLAUDE_PLUGIN_ROOT}/hooks/inject-x.mjs\\"" }] }\n    ]\n  }\n}\n',
    ".harness/guardcore/project/hooks/inject-x.mjs": "// custom session hook (marker: INJECT_X)\nprocess.exit(0);\n",
  });
  try {
...
test("build-skills 크래시 가드 — phase 소스 0개면 조용한 ENOENT 대신 명확한 에러+exit≠0", () => {
  ...
  const { work, r } = buildInline("bs-nophase-", "ghost", {
    ".harness/ghost/config.yaml": "core: ghost\nphases:\n  - spec\n", // spec 소스 없음 + ghost 묶음 없음 → 컴파일 0개.
  });
  try {
...
test("build-skills 기본 게이트 훅 — 크로스OS rel + 문법·동작(비손상 deny)", () => {
  ...
  const { work, r } = buildInline("bs-gate-", "gatecore", {
    ".harness/gatecore/config.yaml": "harness: gatecore\nsync: strict\nphases:\n  - spec\n",
    ".harness/gatecore/project/phases/spec.md": "---\nname: spec\ndesc: 명세.\nconsumes: ~\nproduces: spec\ngate: \"무언가 green\"\n---\n본문.\n",
  });
  ...
  try {
    ...
    // 활성 하네스 게이트 헬퍼가 같이 emit 됐는가(gate-review 가 import 함).
    assert.ok(existsSync(join(work, ".agentoppa/plugins/gatecore/hooks/harness-active.mjs")), "활성 하네스 게이트 헬퍼(harness-active.mjs) 미생성");
    assert.ok(src.includes("isActiveHarness"), "게이트 훅에 활성 하네스 게이트가 없음(.harness-main 무시 회귀)");
    ...
    const editInput = '{"tool_name":"Edit","tool_input":{"file_path":"src/x.js"}}';

    // (게이트 OFF) .harness-main 이 없으면(또는 다른 하네스면) 조용히 빠진다 — deny 없음(단일 활성 하네스).
    const off = spawnSync(process.execPath, [hook], {
      input: editInput, encoding: "utf8", env: { ...process.env, CLAUDE_PROJECT_DIR: mock, HARNESS_MAIN: "" },
    });
    assert.ok(!/"permissionDecision":"deny"/.test(off.stdout), "활성 하네스가 아닌데 게이트가 막음 — 단일 활성 하네스 위반(동시 다중 차단)");

    // (게이트 ON) .harness-main 이 이 하네스를 가리키면: 기존 src/ 파일 Edit 시도 → deny.
    writeFileSync(join(mock, ".harness-main"), "gatecore\n");
    const h = spawnSync(process.execPath, [hook], {
      input: editInput, encoding: "utf8", env: { ...process.env, CLAUDE_PROJECT_DIR: mock, HARNESS_MAIN: "" },
    });
    assert.ok(/"permissionDecision":"deny"/.test(h.stdout), "활성 하네스인데 기존 src/ 파일 수정을 비손상 게이트가 막지 않음");
  } finally {
    rmSync(work, { recursive: true, force: true });
...
test("build-skills 선택 빈자리 꼬리(요청6) — optional 능력·값은 '건너뛰고 명시' 변형, 필수는 '멈춤' 유지", () => {
  ...
  const { work, r } = buildInline("bs-optslot-", "optcore", {
    ".harness/optcore/config.yaml": "harness: optcore\nphases:\n  - solo\n",
    ".harness/optcore/project/phases/solo.md":
