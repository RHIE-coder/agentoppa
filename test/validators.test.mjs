...
const CASES = [
  ...
  },
  {
    ...
    red: ".agentoppa/fixtures/agent-engineer-config/red/.harness/hn/config.yaml",
    green: ".agentoppa/fixtures/agent-engineer-config/green/.harness/hn/config.yaml",
  },
  {
    ...
    validator: "plugins/agentoppa/skills/agent-engineer/scripts/validate.mjs",
    red: ".agentoppa/fixtures/agent-engineer-inline-comment/red/.harness/hn/config.yaml",
    green: ".agentoppa/fixtures/agent-engineer-inline-comment/green/.harness/hn/config.yaml",
  },
  {
    ...
    validator: "plugins/agentoppa/skills/agent-engineer/scripts/validate.mjs",
    red: ".agentoppa/fixtures/agent-engineer-bindings/red/.harness/hn/config.yaml",
    green: ".agentoppa/fixtures/agent-engineer-bindings/green/.harness/hn/config.yaml",
  },
  {
    ...
    validator: "plugins/agentoppa/skills/agent-engineer/scripts/validate.mjs",
    red: ".agentoppa/fixtures/agent-engineer-blockscalar/red/.harness/hn/config.yaml",
    green: ".agentoppa/fixtures/agent-engineer-blockscalar/green/.harness/hn/config.yaml",
  },
  {
    name: "intent-interview — 차단 미해결인데 status=ready 점검",
    validator: "plugins/agentoppa/skills/intent-interview/scripts/validate.mjs",
    red: ".agentoppa/fixtures/intent-interview/red/.harness/hn/intent.md",
    green: ".agentoppa/fixtures/intent-interview/green/.harness/hn/intent.md",
  },
  {
...
function writeHarness(root, cfg, artifacts) {
  const h = join(root, ".harness", "x"); // 단일 활성 하네스: 프로젝트 층은 .harness/<하네스>/ 아래(config 의 harness: x).
...
test("agent-engineer/validate 신선도(E1) — lock 없으면 초기화, 안 바뀌면 갱신, 바뀌면 stale", () => {
  ...
  try {
    ...
    const lockPath = join(work, ".harness/x/artifacts/fixt/lock.json");
    ...
    // 3) 산출물 변경 → stale 감지.
    writeFileSync(join(work, ".harness/x/artifacts/fixt/spec.md"), "명세 v2 (내용 바뀜)\n");
    r = runValidate(cfgPath);
...
test("agent-engineer/validate 프로젝트 확장(요청3) — project/impl/validate-extra.mjs 를 이어서 실행·결과 합침", () => {
  ...
  try {
    const cfgPath = writeHarness(work, "harness: x\nphases:\n  - spec\n");
    const extraPath = join(work, ".harness/x/project/impl/validate-extra.mjs");
