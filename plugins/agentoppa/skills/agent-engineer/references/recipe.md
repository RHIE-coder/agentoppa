...
phases:                       # 순서 = 흐름. 각 이름 → 워크플로우의 phase
  ...
  - tdd
  - review
values:                       # (선택) 값-빈자리 채움. requires의 값-빈자리 / {프로젝트값} → 컴파일 때 본문에 박힘
...
- 단독 하네스면 이름은 `.harness/project/phases/<name>.md`로 푼다 — 없으면 `phases.md` 양식대로 그 자리에 둔다. `core:`를 가리키면 그 Core 묶음의 phase를 쓴다.
...
(phase 자체(워크플로우)는 유저 Core의 콘텐츠다 — 단독 하네스면 `.harness/project/phases/`. AgentOppa는 샘플/프리셋을 싣지 않는다. 순서·강도·`core:`·바인딩까지가 여기, 컴파일 결과를 양쪽 도구로 묶는 포장은 `ccc-plugin`.)
