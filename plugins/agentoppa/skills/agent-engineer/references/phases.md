...
---
.harness/artifacts/<작업폴더>/spec.md 기준 시나리오를 만든다. `test_command`(값) 로 단위부터 통과시킨다.
...
산출 .harness/artifacts/<작업폴더>/e2e.md (헤더 phase: e2e, status: ready, inputs: [spec]). → (종착)

위 경로의 `<작업폴더>` 는 빌드 때 박지 않는다 — 실행 시 정한다: `config.yaml` 의 `feature:` 값이 있으면 그것, 없으면 현재 git 브랜치 이름, 둘 다 없으면 `default`.
```
...
| 슬롯 | 채워지는 값 | 해석 주체 |
|---|---|---|
| `{역할}` (예 `{spec}`) | 산출물 경로 `.harness/artifacts/<작업폴더>/spec.md` — 역할이름은 박고, `<작업폴더>`는 (박지 않음) "실행 시 `config.feature`→git브랜치→`default` 로 풀라" 산문 | contract (경로) + **스킬 본문이 실행 시** (작업폴더) |
