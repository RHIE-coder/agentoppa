...
phase 한 장이 *어떻게 생겼는지*만 정의한다(스키마 — 도메인 무관·고정). **구체 phase(spec·review…)는 여기 없다** → 그것들은 재사용 워크플로우(=Core)에 사는 콘텐츠다. 단독 하네스(standalone)면 그 phase 파일이 `.harness/<하네스>/project/phases/<name>.md`에 있고, Core를 가리키는 모드면 Core 묶음 안에 있다. 어떻게 이어지는지는 `contract.md`, 어떤 순서·옵션·`core:` 배선인지는 `recipe.md`.
...
컴파일된 SKILL.md 본문(역할이름·next는 박히고, 작업폴더·값·능력은 런타임-읽기 산문으로):
...
---
.harness/<하네스>/artifacts/<작업폴더>/spec.md 기준 시나리오를 만든다. `test_command`(값) 로 단위부터 통과시킨다.
  └ 이 값의 알맹이는 `.harness/<하네스>/config.yaml` 의 `values: test_command:` 가 담은 값이다.
...
그다음 **e2e-runner** 능력으로 실제 흐름을 돌려 통과를 확인한다.
  └ 이 능력의 구현은 `.harness/<하네스>/config.yaml` 의 `bindings: e2e-runner:` 가 가리키는 값이고,
...
산출 .harness/<하네스>/artifacts/<작업폴더>/e2e.md (헤더 phase: e2e, status: ready, inputs: [spec]). → (종착)

위 경로의 `<작업폴더>` 는 빌드 때 박지 않는다 — 실행 시 정한다: `.harness/<하네스>/config.yaml` 의 `feature:` 값이 있으면 그것, 없으면 현재 git 브랜치 이름, 둘 다 없으면 `default`.
```
...
## 본문 슬롯 (컴파일러가 처리 — next·역할이름은 박고, 작업폴더·값·능력은 런타임-읽기 산문으로)

| 슬롯 | 채워지는 값 | 해석 주체 |
|---|---|---|
| `{역할}` (예 `{spec}`) | 산출물 경로 `.harness/<하네스>/artifacts/<작업폴더>/spec.md` — 역할이름은 박고, `<작업폴더>`는 (박지 않음) "실행 시 `config.feature`→git브랜치→`default` 로 풀라" 산문 | contract (경로) + **스킬 본문이 실행 시** (작업폴더) |
...
*실제 phase 모음(워크플로우)은 유저가 만든 재사용 Core의 콘텐츠다 — 단독 하네스면 `.harness/<하네스>/project/phases/`에 둔다. 빈 골격은 `../template.md`, 견본은 `examples/sample.md`. (AgentOppa는 phase를 샘플로 싣지 않는다 — 콘텐츠는 유저 Core에서 온다.)*
