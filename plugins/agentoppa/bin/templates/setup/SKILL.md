---
name: setup
description: 이 하네스 프레임워크({{CORE}})를 새 프로젝트에 처음 붙일 때 사용. .harness/config.yaml 이 없거나 빈자리(values·bindings)가 안 채워졌을 때 깔아 준다. "이 하네스 붙여줘", "셋업 해줘", "{{CORE}} 설정", 또는 이 Core 의 단계 스킬을 처음 쓰는데 .harness 가 아직 없을 때. AgentOppa 없이 이 플러그인만으로 동작.
---
